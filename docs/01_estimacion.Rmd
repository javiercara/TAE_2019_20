...
- El término lineal se emplea porque la Ec. \@ref(eq:modelo2) es una función lineal de los parámetros $\beta_0$, $\beta_1$, $\beta_2$ y $\beta_3$. 
- Modelos en apariencia complicados pueden ser considerados como un modelo lineal.
- polinomios:
...
La Ec. \@ref(eq:modelo2) se suele escribir en notación matricial. Para ello escribimos la ecuación para todos los datos disponibles:
...
\begin{equation}
\hat y = X \hat \beta = X (X^TX)^{-1}XY = H y
\end{equation}
...
Es conveniente medir como de bueno es el ajuste del modelo y los datos. La manera mas usual es utilizar el coeficiente de determinación o $R^2$:
